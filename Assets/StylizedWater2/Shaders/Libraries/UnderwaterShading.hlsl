...
#if !SHADERGRAPH_PREVIEW && defined(_UNDERWATER_ENABLED)
...
float3 SampleCaustics(float3 depthPos, float2 time, float tiling)
{
	...
	float3 caustics1 = SAMPLE_TEX(_CausticsTex, sampler_CausticsTex, depthPos.xz* tiling + (time.xy )).rgb;
	float3 caustics2 = SAMPLE_TEX(_CausticsTex, sampler_CausticsTex, (depthPos.xz* tiling * 0.8) -(time.xy)).rgb;
...
void ApplyUnderwaterShading(inout float3 color, inout float alpha, float3 worldPos, float3 normal, float3 viewDir, float3 shallowColor, float3 deepColor, float mask)
{
	...
	ComputeDensity(worldPos, _WaterLevel, _WorldSpaceCameraPos.y, fogDistFactor, fogHeightFactor);
...
void ClipSurface(float4 screenPos, float clipZ, float vFace)
{
	...
	float underwaterMask = SAMPLE_RT(_UnderwaterMask, sampler_UnderwaterMask, (screenPos.xy / screenPos.w)).r;
